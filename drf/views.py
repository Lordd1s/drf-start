...
def props(request: Request) -> Response:
    if request.method == "GET":
        data = serializers.ProposeModelSerializer(
            models.ProposeModel.objects.all(), many=True
        ).data
        ...
        try:
            ...
            return Response(
                data={"message": "Invalid email"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        models.ProposeModel.objects.create(
            full_name=full_name, email=email, suggestion=suggestion, date=formatted_date
        )
...
def props_del_upd(request: Request, pk) -> Response:
    ...
    if request.method == "GET":
        ...
        try:
            data.delete()
            return Response(
                data={"message": "delete successful"}, status=status.HTTP_204_NO_CONTENT
            )
        ...
        try:
            data.save()
            return Response(
                data={"message": "Update successful"}, status=status.HTTP_200_OK
            )
        except Exception as e:
            return Response(
...
def news(request: Request) -> Response:
    if request.method == "GET":
        news = serializers.NewsModelSerializerMany(
            models.News.objects.all(), many=True
        ).data
        ...
        if not category_names or not title or not description:
            return Response(
                data={"message": "Category, title, and description are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def news_one(request: Request, pk: str) -> Response:
    if request.method == "GET":
        new = serializers.NewsModelSerializerOne(
            models.News.objects.get(id=int(pk)), many=False
        ).data
